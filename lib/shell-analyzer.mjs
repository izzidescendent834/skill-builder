...
export function analyzeShellHistory(historyPath) {
  ...
  for (const { command, count } of repeated.slice(0, 5)) {
    const slug = command.split(/\s+/).slice(0, 3).join("-").replace(/[^a-z0-9-]/gi, "").toLowerCase().slice(0, 30);
    suggestions.push({
      id: `shell-${slug}`,
  ...
  for (const { sequence, count } of sequences.slice(0, 5)) {
    const slug = sequence.split(/\s+/).slice(0, 3).join("-").replace(/[^a-z0-9-]/gi, "").toLowerCase().slice(0, 30);
    suggestions.push({
      id: `workflow-${slug}`,
  ...
  for (const { pattern, variations, total, examples } of parameterizable.slice(0, 5)) {
    const slug = pattern.split(/\s+/).slice(0, 3).join("-").replace(/[^a-z0-9-]/gi, "").toLowerCase().slice(0, 30);
    suggestions.push({
      id: `template-${slug}`,
