  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      switch (route.provider) {
        case "google":
          result = await callGoogle(prompt, route.model, keys.google);
          break;
        case "anthropic":
          result = await callAnthropic(prompt, route.model, keys.anthropic);
          break;
        case "openai":
          result = await callOpenai(prompt, route.model, keys.openai);
          break;
        case "openrouter":
          result = await callOpenrouter(prompt, route.model, keys.openrouter);
          break;
        default:
